...
namespace VoxelSystem
{
	public partial class VoxelEditorWindow
	{
		...
		static void DrawWireCube_InWorld(Transform transform, Vector3 origin, Vector3 size)
		{
			...
			Handles.DrawLine(p000, p010);
			Handles.DrawLine(p010, p110);
			Handles.DrawLine(p110, p100);
			Handles.DrawLine(p100, p000);
			Handles.DrawLine(p000, p001);
			Handles.DrawLine(p010, p011);
			Handles.DrawLine(p100, p101);
			Handles.DrawLine(p110, p111);
			Handles.DrawLine(p001, p011);
			Handles.DrawLine(p011, p111);
			Handles.DrawLine(p111, p101);
			Handles.DrawLine(p101, p001);
		}
		...
		static void DrawPolygon(Vector3[] points, int drawableleFragments = 1, bool closeLine = true)
		{
			if (points.Length <= 1)
			{ return; }
			if (drawableleFragments < 1)
			{ drawableleFragments = 1; }
