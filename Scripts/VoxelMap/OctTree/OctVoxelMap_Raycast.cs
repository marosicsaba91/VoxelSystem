...
{ 
	partial class OctVoxelMap
	{
		...
		static bool Raycast(Ray globalRay, out InVoxelPoint hit, OctVoxelMap map, Matrix4x4 matrix, bool returnOutsideVoxel)
		{
			Ray localRay = globalRay.Transform(matrix);

			// Try Find the entry point
			if (FindEntryPointToVoxelMap(localRay, out InVoxelPoint voxelMapEntry, map.CanvasSize))
			{
		...
		static bool RaycastInside(InVoxelPoint entry, out InVoxelPoint hit, Vector3 rayDirection, OctVoxelMap map, bool returnOutsideVoxel)
		{
			...
			if (map.Get(e.x, e.y, e.z).IsFilled())
			{
				hit = entry;
				return true;
			}
