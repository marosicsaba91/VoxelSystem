...
namespace VoxelSystem
{
    ...
    {
        public bool Raycast(Ray ray, out InVoxelPoint hit, Transform voxelTransform, bool returnOutsideVoxel = false)
        {
            if (voxelTransform == null)
            {
                hit = new InVoxelPoint();
                return false;
            }
        ...
        static bool Raycast(Ray globalRay, out InVoxelPoint hit, VoxelMap map, Matrix4x4 matrix, bool returnOutsideVoxel)
        {
            ...
            if (FindEntryPointToVoxelMap(localRay, out InVoxelPoint voxelMapEntry, map.Size))
            {
        ...
        static bool FindEntryPointToVoxelMap(Ray ray, out InVoxelPoint hit, Vector3Int mapSize)
        {
            ...
            for (int i = 0; i < sides.Length; i++)
            {
                ...
                if (entryPoint.x <= -epsylon || entryPoint.x >= mapSize.x + epsylon) continue;
                if (entryPoint.y <= -epsylon || entryPoint.y >= mapSize.y + epsylon) continue;
                if (entryPoint.z <= -epsylon || entryPoint.z >= mapSize.z + epsylon) continue;
        ...
        static bool RaycastInside(InVoxelPoint entry, out InVoxelPoint hit, Vector3 rayDirection, VoxelMap map, bool returnOutsideVoxel)
        {
            ...
            if (map.Get(entry.voxel).IsFilled)
            {
                hit = entry;
                return true;
            }

            InVoxelPoint cursor = new();
            // In the Cube
            ...
            while(true)
            {
                ...
                if (map.Get(lastFoundVoxel.x, lastFoundVoxel.y, lastFoundVoxel.z).IsFilled)
                {
                    cursor.voxel = returnOutsideVoxel ? cursor.voxel : cursor.voxel + cursor.side.ToVectorInt();
                    cursor.side = returnOutsideVoxel ? cursor.side : cursor.side.Opposite();
                    hit = cursor;
