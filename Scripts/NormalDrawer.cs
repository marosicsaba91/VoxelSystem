...
public class NormalDrawer : MonoBehaviour
{
    ...
    void DrawTriangles(Mesh mesh)
    {
        ...
        if (!drawNormalsFromWinding && !drawMeanVertexNormals && !drawWindings && !drawOutline) return;
    ...
    {
        if (mesh == null || mesh.vertices == null || mesh.normals == null) return;
        if (!drawVertexNormals) return;
